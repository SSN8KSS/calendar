...
const generateSeed = (start, j) => {
  const writeBookingsByUser = fs.createWriteStream(`./bookings_by_hotel${j}.csv`);
  const writeTenMillionBookingsByUser = (writer, encoding, callback) => {
    let i = 1000000;
    let id = start;
    ...
    function write() {
      ...
      do {
        ...
        const creditCard = faker.random.number(10000000, 90000000);
        const userId = i;

        const data = `${bookingId},${bookingHotelId},${bookingService},${adults},${children},${checkIn},${checkOut},${nightlyPrice},${cleaningPrice}, ${creditCard}, ${userId}\n`;
  ...
  writeTenMillionBookingsByUser(writeBookingsByUser, 'utf-8', () => {
    writeBookingsByUser.end();
  });
